...
struct XOR_binary_trie{
    ...
    int MAX_BIT = 18;
    // Adds count times of the number in to the trie
    ...
    // check count occurence of the number in the trie
    bool check(int x,int count = 1){
        node* root = R;
        for(int i = MAX_BIT;i>=0;i--){
            int bit = x>>i&1;
            node* next = root->next[bit];
            // If next node is null or has count less than count return false
            if(next == NULL) return 0;
            if(next->cnt < count) return 0;
